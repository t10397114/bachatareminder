...
async def ask_admin(group, class_time):
    print(f"[ask_admin] Спрашиваем про: {group['name']}, чат: {group['chat_id']}, ADMIN_ID: {ADMIN_ID}", flush=True)
    ...
    try:
        ...
        print(f"[ask_admin] Ошибка при отправке сообщения: {e}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_key in processed_callbacks:
        print(f"[callback] Already processed callback: {callback_key}")
        return
    ...
    try:
        ...
        print(f"[callback] Ошибка при отправке сообщения: {e}")
